...
class CompactDerivative:
    def __init__(self, x, dtype: DerivType, method: CFDSolve):
        self.x = x
        self.N = len(x)
        self.dx = x[1] - x[0]
        self.type = type
        ...
        if dtype == DerivType.D1_JT4:
            ...
            self.bands = (2, 2)
            self.denom = 1.0 / self.dx
        elif dtype == DerivType.D1_SP4:
            self.ab, self.B = self._init_D1_SP4()
            self.bandwidth = 2
            self.bands = (2, 2)
            self.denom = 1.0 / self.dx
        elif dtype == DerivType.D1_DE4:
            self.ab, self.B = self._init_D1_De4()
            self.bandwidth = 2
            self.bands = (2, 2)
            self.denom = 1.0 / self.dx
        else:
            raise ValueError(f"Unknown derivative type = {dtype}")

        if method == CFDSolve.LUSOLVE:
            self.lu_factorization = blu.lu_banded(
                self.bands,
                self.ab,
                overwrite_ab=self.overwrite,
                check_finite=self.checkf,
            )
        elif method == CFDSolve.D_INV:
            nb = self.bandwidth
            A = utils.banded_to_full(self.ab, nb, nb, self.N, self.N)
            Ainv = inv(A)
            self.D = np.matmul(Ainv, self.B)
        elif method == CFDSolve.D_LU:
            self.D = solve_banded(self.bands, self.ab, self.B)
        elif method == CFDSolve.SCIPY or method == CFDSolve.PENTAPY:
            pass
        else:
    ...
    def grad(self, f):
        if self.method == CFDSolve.D_LU or self.method == CFDSolve.D_INV:
            if self.D is None:
                raise ValueError(
                    "Matrix D has not been initialized for method '{}'.".format(
                        self.method
                    )
                )
            ...
            if self.lu_factorization is None:
                raise ValueError(
                    "LU factorization has not been initialized for method 'LUSOLVE'."
                )
            return (
                blu.lu_solve_banded(
                    self.lu_factorization, rhs, overwrite_b=True, check_finite=True
                )
                * self.denom
            )
        elif self.method == CFDSolve.PENTAPY:
            if self.bands != (2, 2):
                raise ValueError("PENTAPY method requires bands to be (2, 2).")
            rhs = np.matmul(self.B, f)
            return pp.solve(self.ab, rhs, is_flat=True, index_row_wise=False, solver=1)
        else:
